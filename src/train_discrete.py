...
def train(args):
    ...
    val_dataset = DiscreteInferenceDataset(args.val_dataset_path, visual2index)
    ...
    model = nn.DataParallel(SpatialDiscreteBert(config, args.bert_name)).to(device)
    # Loss and optimizer
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Trains a Discrete Spatial-BERT model."
    )
    parser.add_argument(
        "--checkpoint_path",
        type=str,
